...
function addToCartClicked(event) {
    console.log("call addtocart method")
    ...
    addItemToCart(title,quantity, price)
    updateCartTotal()
}

function addItemToCart(title, quantity,price) {

    var cartRow = document.createElement('div')
    cartRow.classList.add('cart-row')
    var cartItems = document.getElementsByClassName('cart-items')[0]
    var cartItemNames = cartItems.getElementsByClassName('cart-item-title')
    console.log("cart item name is "+cartItemNames.length)
    for (var i = 0; i < cartItemNames.length; i++) {
        console.log("inside for loop "+cartItemNames[i].innerText)
        if (cartItemNames[i].innerText == title) {
            alert('This item is already added to the cart.Please go to the cart if you want to  add more.')
            return
        }
    }
